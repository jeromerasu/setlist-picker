...
def test_display_name_override_takes_priority() -> None:
    user = _make_user(display_name="Real Name")
...
def test_display_name_falls_back_to_member_literal_when_no_display_name() -> None:
...
def test_resolve_member_out_batch_returns_list() -> None:
    user1 = _make_user(display_name="User One")
    user2 = _make_user(display_name="User Two")
    m1 = _make_member(user_id=user1.id)
    m2 = _make_member(user_id=user2.id)
    results = resolve_member_out_batch([(m1, user1), (m2, user2)])
    assert len(results) == 2
    assert {r.display_name for r in results} == {"User One", "User Two"}
